  ...
  describe('resolveNoShow', () => {
    ...
    beforeEach(() => {
      prisma.noShowReport = {
        ...(prisma.noShowReport ?? {}),
        findUnique: jest.fn(),
      } as typeof prisma.noShowReport;
      prisma.strike = { findUnique: jest.fn() };
    ...
    it('confirms a no-show: strikes by worker profile id, marks booking NO_SHOW', async () => {
      (
        prisma as Record<string, unknown & { findUnique: jest.Mock }>
      ).noShowReport.findUnique = jest.fn().mockResolvedValue(report);
      (
        prisma as Record<string, unknown & { findUnique: jest.Mock }>
      ).strike.findUnique = jest.fn().mockResolvedValue(null);
      tx.workerProfile.update.mockResolvedValue({
    ...
    it('throws NotFoundException when the report does not exist', async () => {
      (
        prisma as Record<string, unknown & { findUnique: jest.Mock }>
      ).noShowReport.findUnique = jest.fn().mockResolvedValue(null);
